...
(function(){
    ...
    * @method autoDetectOS */
    function autoDetectOS () {
        var detected_manufacturer;
        var detected_configuration;
        var found = false;
        ...
        if ( formattedAppVersion.indexOf("iPad") !== -1 ) {
            detected_manufacturer = "mobile";
            detected_configuration = "ipad";
            found = true;
        } else if ( formattedAppVersion.indexOf("iPod") !== -1) {
            detected_manufacturer = "mobile";
            detected_configuration = "ipod";
            found = true;
        } else if (formattedAppVersion.indexOf("Android") !== -1) {
            detected_manufacturer = "mobile";
            detected_configuration = "android";
            found = true;
        } else if (formattedAppVersion.indexOf("iPhone") !== -1 ) {
            detected_manufacturer = "mobile";
            detected_configuration = "iphone";
            found = true;
        } else if (formattedAppVersion.indexOf("X11") !== -1) {
            detected_manufacturer = "linux";
            detected_configuration = "nautilus";
            found = true;
            ...
            detected_manufacturer = (osver && osver <= 10.4) ? "osx" : "osx105";
            if (osver) {
                if (osver === 10.4) {
                    detected_configuration = "osx_tiger";
                    found = true;
                } else if (osver === 10.5) {
                    detected_configuration = "osx_leopard";
                    found = true;
                } else if (osver === 10.6) {
                    detected_configuration = "osx_snowleopard";
                    found = true;
                } else if (osver === 10.7) {
                    detected_configuration = "osx_lion";
                    found = true;
                } else if (osver === 10.8) {
                    detected_configuration = "osx_mountainlion";
                    found = true;
                } else if (osver === 10.9) {
                    detected_configuration = "osx_mavericks";
                    found = true;
                } else if (osver === 10.10) {
                    detected_configuration = "osx_yosemite";
                    found = true;
                } else if (osver === 10.11) {
                    detected_configuration = "osx_elcapitan";
                    found = true;
                } else if (osver >= 10.12) {
                    detected_configuration = "osx_sierra";
                    found = true;
                }
            }
        } else if (formattedAppVersion.indexOf("Win") !== -1) {
            detected_manufacturer = "microsoft";
            oskey = navigator.oscpu ?  navigator.oscpu : formattedAppVersion;
            if (oskey.indexOf("Windows 98") !== -1) {
                detected_configuration = "win98";
            } else if (oskey.indexOf("Windows NT") !== -1) {
                osmatch = oskey.match(/NT\s+([0-9]+\.[0-9]+)/);
                osver = parseFloat(osmatch[1]);
                if (osver < 5.1) {
                    detected_configuration = "win2000";
                    found = true;
                } else if (osver < 5.2) {
                    detected_configuration = "winxp";
                    found = true;
                } else if (osver < 6.0) {
                    detected_configuration = "win2003";
                    found = true;
                } else if (osver < 6.1) {
                    detected_configuration = "winvista";
                    found = true;
                } else if (osver < 6.2) {
                    detected_configuration = "win7";
                    found = true;
                } else if (osver < 6.3) {
                    detected_configuration = "win8";
                    found = true;
                } else if (osver === 10.0) {
                    detected_configuration = "win10";
                    found = true;
                }
            }
        }
        ...
        return {
            configuration: detected_configuration,
