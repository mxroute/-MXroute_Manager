...
define(
    ...
    function(angular) {
        ...
        /**
         * Directive that checks an input field for a maximum value, but only if another field is not set to unlimited
         * @attribute {Integer} emailMaxQuota  - The maximum value allowed for the field
         ...
         *          <input type="number" name="quota" email-max-quota="1234567890" email-quota-type="formName.quotaType">
         *          <span>MB</span>
         *     </div>
         *     <div>
        ...
        module.directive("emailMaxQuota", function () {
            return {
                require: "ngModel",
                restrict: "A",
                link: function ( scope, element, attrs, ngModel ) { // eslint-disable-line no-unused-vars

                    ngModel.$validators.emailMaxQuota = function(model, view) { // eslint-disable-line no-unused-vars
                        ...
                        if( quotaTypeModel.$viewValue !== "unlimited" ) {
                            if( ngModel.$isEmpty(view) ) {
                                ...
                                var maxQuota = parseInt(attrs.emailMaxQuota, 10);
                                return view <= maxQuota;
                            }
                        }
                        else {
                            return true;
