...
function searchpage(searchBoxEl) {
    ...
    for (var i = 0; i < liEls.length; i++) {
        ...
        if (!ElementTextCache[liEls[i].id] || ElementTextCache[liEls[i].id] == "") {
            ...
            var additional_search_text_el = YAHOO.util.Dom.getFirstChildBy(liEls[i], function(e) {
                return YAHOO.util.Dom.hasClass(e, "additional_search_text");
            });
...
YAHOO.util.Event.onDOMReady(clearsearch);
...
function focus_quickjump() {
    ...
    if (quickJumpEl.offsetHeight !== 0 || quickJumpEl.offsetWidth !== 0) {
        quickJumpEl.focus();
    }
