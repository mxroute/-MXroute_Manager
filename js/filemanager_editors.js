...
function confirm_close(clicked_el) {
    var to_confirm = window.CODEWINDOW ? CODEWINDOW.value !== CODEWINDOW.defaultValue : wp_current_obj.getSubmitValue() !== LAST_SAVED_VALUE;

    if (to_confirm) {
        var confirmation = new CPANEL.ajax.Common_Dialog();
        confirmation.setBody(LEXICON.confirm_close);
